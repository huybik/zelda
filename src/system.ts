...
export class Controls {
  ...
  // mobile parts
  movementInput: Vector2 = new Vector2(0, 0);
  cameraInput: Vector2 = new Vector2(0, 0);
  movementTouchId: number | null = null;
  ...
  initMobileControls(): void {
    ...
    movementJoystick.addEventListener(
      "touchstart",
      this.onMovementTouchStart.bind(this),
      { passive: false }
    );
    movementJoystick.addEventListener(
      "touchmove",
      this.onMovementTouchMove.bind(this),
      { passive: false }
    );
    movementJoystick.addEventListener(
      "touchend",
      this.onMovementTouchEnd.bind(this)
    );

    cameraJoystick.addEventListener(
      "touchstart",
      this.onCameraTouchStart.bind(this),
      { passive: false }
    );
    cameraJoystick.addEventListener(
      "touchmove",
      this.onCameraTouchMove.bind(this),
      { passive: false }
    );
    cameraJoystick.addEventListener(
      "touchend",
      this.onCameraTouchEnd.bind(this)
    );

    interactButton.addEventListener(
      "touchstart",
      this.onInteractTouchStart.bind(this)
    );
    attackButton.addEventListener(
      "touchstart",
      this.onAttackTouchStart.bind(this)
    );
    attackButton.addEventListener("touchend", this.onAttackTouchEnd.bind(this));

    inventoryButton.addEventListener("touchstart", () => {
    ...
    journalButton.addEventListener("touchstart", () => {
      if (this.game?.interactionSystem?.isChatOpen) return;
  ...
  onMovementTouchStart(event: TouchEvent): void {
    if (this.movementTouchId !== null) return;
    const touch = event.changedTouches[0];
    this.movementTouchId = touch.identifier;
  }

  onMovementTouchMove(event: TouchEvent): void {
    if (this.movementTouchId === null) return;
    const touch = Array.from(event.changedTouches).find(
      (t) => t.identifier === this.movementTouchId
    );
    if (!touch) return;
    const joystick = event.target as HTMLElement;
    ...
    if (magnitude > 1) {
      this.movementInput.x = dx / magnitude;
      this.movementInput.y = dy / magnitude;
    } else {
      this.movementInput.x = dx;
      this.movementInput.y = dy;
    }
  }

  onMovementTouchEnd(event: TouchEvent): void {
    const touch = Array.from(event.changedTouches).find(
      (t) => t.identifier === this.movementTouchId
    );
    if (touch) {
  ...
  onCameraTouchMove(event: TouchEvent): void {
    if (this.cameraTouchId === null) return;
    const touch = Array.from(event.changedTouches).find(
      (t) => t.identifier === this.cameraTouchId
    );
    if (!touch) return;
    const joystick = event.target as HTMLElement;
    const rect = joystick.getBoundingClientRect();
    ...
    const magnitude = Math.sqrt(dx * dx + dy * dy);
    if (magnitude > 1) {
      this.cameraInput.x = dx / magnitude;
      this.cameraInput.y = dy / magnitude;
    } else {
      this.cameraInput.x = dx;
      this.cameraInput.y = dy;
    }
  }

  onCameraTouchEnd(event: TouchEvent): void {
    const touch = Array.from(event.changedTouches).find(
      (t) => t.identifier === this.cameraTouchId
    );
    if (touch) {
  ...
  onInteractTouchStart(event: TouchEvent): void {
    event.preventDefault();
    this.moveState.interact = true;
  }

  onAttackTouchStart(event: TouchEvent): void {
    event.preventDefault();
    this.moveState.attack = true;
  }

  onAttackTouchEnd(event: TouchEvent): void {
  ...
  update(deltaTime: number): void {
    //mobile update
    if (this.game?.isMobileDevice()) {
      this.moveState.forward = this.movementInput.y;
      this.moveState.right = this.movementInput.x;

      const cameraSpeed = 5; // Adjust sensitivity
      this.mouse.dx += this.cameraInput.x * cameraSpeed;
      this.mouse.dy += this.cameraInput.y * cameraSpeed;
    } else {
      const W = this.keys["KeyW"] || this.keys["ArrowUp"];
      const S = this.keys["KeyS"] || this.keys["ArrowDown"];
      const D = this.keys["KeyD"] || this.keys["ArrowRight"];
